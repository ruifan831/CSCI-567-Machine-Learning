...
def binary_train(X, y, loss="perceptron", w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    if loss == "perceptron":
        ...
        for i in range(max_iterations):
            ...
            gradient_w = np.sum((np.where((pred*y_true)<=0,-1,0)*y_true).reshape(-1,1)*X,axis=0)/X.shape[0]
            gradient_b = np.sum(np.where(pred*y_true<=0,-1,0)*y_true,axis=0)/X.shape[0]
        ...
        for i in range(max_iterations):
            ...
            y_true = np.where(y==1,1,-1)
            z=pred*y_true
            gradient_w = np.sum((-np.exp(-z)/(1+np.exp(-z))*y_true).reshape(-1,1)*X,axis=0)/X.shape[0]
            gradient_b = np.sum(-np.exp(-z)/(1+np.exp(-z))*y_true,axis=0)/X.shape[0]
            w = w - step_size*gradient_w
            b = b - step_size*gradient_b
...
def binary_predict(X, w, b):
    ...
    #############################################################
    preds = np.where(X.dot(w.T)+b<=0,0,1)
...
def multiclass_train(X, y, C,
    ...
    if gd_type == "sgd":
        ...
        for it in range(max_iterations):
            ...
            z_bar = z - np.max(z)
            temp = np.exp(z_bar)
            temp = temp/np.sum(temp) - y_true
            
            w = w - step_size*(np.dot(temp.reshape(-1,1),x.reshape(1,-1)))
            b = b - step_size*temp
        ...
        for i in range(max_iterations):
            ...
            z=w.dot(X.T)+b.reshape(-1,1)
            exp = np.exp(z-np.max(z,axis=0))
            grdient_w = (exp/np.sum(exp,axis=0)-y_true).dot(X)/X.shape[0]
            w = w- step_size*grdient_w
            grdient_b = np.sum(exp/np.sum(exp,axis=0)-y_true,axis=1)/X.shape[0]
            b = b - step_size*grdient_b
...
def multiclass_predict(X, w, b):
    ...
    z=w.dot(X.T)+b.reshape(-1,1)
    exp = np.exp(z-np.max(z,axis=0))
    preds = np.argmax(exp/np.sum(exp,axis=0),axis=0)
