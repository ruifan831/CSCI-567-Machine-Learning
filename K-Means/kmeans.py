...
def get_k_means_plus_plus_center_indices(n, n_cluster, x, generator=np.random):
    ...
    for i in range(n_cluster-1):
        cur_distances= []
        for j in range(n):
            cur_distances.append(min([np.linalg.norm(x[j]-x[center])**2 for center in centers]))
        pChoice = generator.rand()
        cdf = np.cumsum(cur_distances)/sum(cur_distances)
        cur_choice = np.min(np.argwhere(cdf>pChoice))
        centers.append(cur_choice)
        # centers.append(np.argmax(cur_distances/sum(cur_distances)))
...
class KMeans():
    ...
    def fit(self, x, centroid_func=get_lloyd_k_means):
        ...
        objective = getObjective(centroids,x,assignments,self.n_cluster)/N
        update=0
        while update<self.max_iter:
            distanceToCenters = np.sum(np.square(x-np.expand_dims(centroids,axis=1)),axis=2)
            assignments = np.argmin(distanceToCenters,axis=0)
            cur_objective = getObjective(centroids,x,assignments,self.n_cluster)/N
...
class KMeansClassifier():
    ...
    def predict(self, x):
        ...
        ##########################################################################
        distanceToCenters = np.sum(np.square(x-np.expand_dims(self.centroids,axis=1)),axis=2)
        result = np.argmin(distanceToCenters,axis=0)
...
def transform_image(image, code_vectors):
    ...
    #     index = np.argmin(list(map(lambda vector:euclidean_distance(image[i],vector),code_vectors)))
    #     image[i] = code_vectors[index]
    # image = image.reshape(row,-1,3)
    return new_image
